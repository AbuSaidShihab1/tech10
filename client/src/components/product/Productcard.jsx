...
<div
  className="border border-[#eee] cursor-pointer rounded-md group hover:shadow-md transition-all duration-200 max-w-[400px] sm:max-w-[320px] md:max-w-[350px] lg:max-w-[400px]"
  style={{ maxHeight: "450px" }}
>
  {/* Image Section */}
  <div className="w-full relative p-3 flex justify-center items-center overflow-hidden">
    <div className="relative w-full h-[150px] xs:h-[160px] sm:h-[180px] md:h-[200px] lg:h-[230px] group">
      {/* First Image */}
      <img
        className="absolute inset-0 w-auto h-full m-auto transition-opacity duration-500 opacity-100 group-hover:opacity-0"
        src={`http://localhost:8080/images/${data?.images[0]}`}
        alt="Product"
      />
      ...
      {data?.images[1] && (
        <img
          ...
          src={`http://localhost:8080/images/${data?.images[1]}`}
          alt="Product"
        />
      )}
    </div>
    ...
    {/* Discount Badge */}
    {data.discount && (
      <span className="px-2 py-1 absolute top-2 left-2 rounded-md text-[10px] sm:text-[12px] font-medium text-white bg-red-500">
        {data.discount}% off
      </span>
    )}
  </div>
